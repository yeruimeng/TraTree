...
class DynamicMCTSLoss(nn.Module):
    ...
    def compute_dynamic_parameters(self):
        ...
        # beta从0.05增加到0.1，增长幅度为0.1
        beta = min(0.1, self.initial_beta + 0.1 * (self.current_step / self.total_steps))
        
        # scaling_factor从initial_scaling_factor降低到50000  webshop 从20降到5
        scaling_factor = max(50000.0, self.initial_scaling_factor / (1 + 0.0005 * self.current_step))
