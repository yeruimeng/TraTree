# #这个肯定是可以跑的 用lora  微调webshop的效果还不错
# ./lora.sh
import os
import argparse
import json
...
def parse_args():
    ...
    parser.add_argument("--data_path", type=str, default="data/sciworld_sft.json")
    parser.add_argument("--output_dir", type=str, default="output/lora_sft_sci_strong_llama2")
...
#     parser.add_argument("--instruction_path", type=str, default="instruction.txt")
#     parser.add_argument("--output_dir", type=str, default="output/lora_sft_weak_llama2_sci")
#     parser.add_argument("--num_train_epochs", type=int, default=3)
#     parser.add_argument("--per_device_train_batch_size", type=int, default=4)
#     parser.add_argument("--gradient_accumulation_steps", type=int, default=2)
#     parser.add_argument("--learning_rate", type=float, default=1e-5)
...
# def preprocess(sources, tokenizer: transformers.PreTrainedTokenizer, instruction: str) -> Dict:
#     conversations = []
#     for source in sources:
#         conversation = []
#         for turn in source:
...
# class SupervisedDataset(Dataset):
#     def __init__(self, raw_data, tokenizer: transformers.PreTrainedTokenizer, instruction: str):
#         super(SupervisedDataset, self).__init__()
#         sources = [example["conversations"] for example in raw_data]
#         data_dict = preprocess(sources, tokenizer, instruction)
#         self.input_ids = data_dict["input_ids"]
#         self.attention_mask = data_dict["attention_mask"]
#         self.labels = data_dict["labels"]
...
#         ret = preprocess([self.raw_data[i]["conversations"]], self.tokenizer, self.instruction)
#         ret = dict(
#             input_ids=ret["input_ids"][0],
#             attention_mask=ret["attention_mask"][0],
#             labels=ret["labels"][0],
...
# def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args) -> Dict:
#     dataset_cls = LazySupervisedDataset if data_args.lazy_preprocess else SupervisedDataset
#     instruction = read_instruction(data_args.instruction_path)
#     print("Loading data...")
#     train_json = json.load(open(data_args.data_path, "r"))
#     train_dataset = dataset_cls(train_json, tokenizer=tokenizer, instruction=instruction)
#     return dict(train_dataset=train_dataset, eval_dataset=None)
...
# 下面是训练LoRA的  可以跑 但是效果还不如原模型  也加了一些层进去  还没试  上面是继续完善的
# import os
# import argparse
# import json
# import torch
