...
#     output_dir="/home/bhui/ML/ruimeng/ETO-main/dpo_output/weak_DPO_sci_lora_step",
#     num_train_epochs=3,
...
# # 政策模型的基础路径（SFT 后的 LLaMA 7B 模型）
# base_model_path = "/home/bhui/ML/ruimeng/ETO-main/output/lora_sft_sci_weak_llama2/merged_model"
...
# # 加载和预处理数据集
# data_path = "/home/bhui/ML/ruimeng/ETO-main/dpo_samples_flattened_3.json"  # 使用扁平化后的JSON文件路径
...
#     且 history 不为空。
#     """
#     has_rejected_labels = any(label != IGNORE_TOKEN_ID for label in example.get('rejected_labels', []))
#     has_non_empty_history = len(example.get('history', [])) > 0
#     return has_rejected_labels and has_non_empty_history
...
#     for message in example.get('prompt', []):
#         role = roles.get(message.get("from"), message.get("from"))
#         conv.append_message(role, message.get("value", ""))
#     for message in example.get('history', []):
#         role = roles.get(message.get("from"), message.get("from"))
#         conv.append_message(role, message.get("value", ""))
...
#     for message in example.get('prompt', []):
#         role = roles.get(message.get("from"), message.get("from"))
#         conv_chosen.append_message(role, message.get("value", ""))
#     for message in example.get('history', []):
#         role = roles.get(message.get("from"), message.get("from"))
#         conv_chosen.append_message(role, message.get("value", ""))
#     for message in example.get('chosen', []):
#         role = roles.get(message.get("from"), message.get("from"))
#         conv_chosen.append_message(role, message.get("value", ""))
...
#     for message in example.get('prompt', []):
#         role = roles.get(message.get("from"), message.get("from"))
#         conv_rejected.append_message(role, message.get("value", ""))
#     for message in example.get('history', []):
#         role = roles.get(message.get("from"), message.get("from"))
#         conv_rejected.append_message(role, message.get("value", ""))
#     for message in example.get('rejected', []):
#         role = roles.get(message.get("from"), message.get("from"))
#         conv_rejected.append_message(role, message.get("value", ""))
...
#         "history": example.get('history', []),  # 确保 history 存在
#     }
...
# # 过滤掉不符合条件的样本
# preprocessed_dataset = preprocessed_dataset.filter(is_valid_sample)

# # 添加日志以确认过滤结果
# print(f"最终过滤后数据集大小: {len(preprocessed_dataset)}")
# if len(preprocessed_dataset) > 0:
#     print("前3个样本示例：")
#     for i in range(min(3, len(preprocessed_dataset))):
...
training_args = TrainingArguments(
    ...
    gradient_accumulation_steps=4,  # 相应调整
    save_steps=500,
    save_total_limit=5,
    learning_rate=learning_rate,
    weight_decay=0.0,
    ...
    max_grad_norm=0.5,  # 设置梯度的最大规范化值
    remove_unused_columns=False,  # 设置为 False
    logging_dir="/home/bhui/ML/ruimeng/ETO-main/logs",  # 添加日志路径
...
# 加载和预处理数据集
data_path = "/home/bhui/ML/ruimeng/ETO-main/dpo_samples_flattened_3.json"
dataset = load_dataset("json", data_files=data_path)["train"]
...
# 手动指定从 checkpoint-1000 恢复训练
checkpoint_dir = "/home/bhui/ML/ruimeng/ETO-main/dpo_output/weak_DPO_sci_lora_step/checkpoint-1000"
