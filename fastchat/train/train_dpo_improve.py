...
from fastchat.train.dpo_trainer_improve import DPOMultiTrainer
...
training_args = TrainingArguments(
    output_dir="/home/bhui/ML/ruimeng/ETO-main/dpo_output/wts_DPO_sci_weak_pair_update_loss",
    num_train_epochs=3,
...
# 加载和预处理数据集
data_path = "/home/bhui/ML/ruimeng/ETO-main/contrastive_trajectoriess.json"
...
def preprocess_function(example):
    ...
    # 构建 chosen（optimal_conversations）
    conv.messages = []
    for j, sentence in enumerate(example['prompt'] + example['optimal_conversations']):
        role = roles[sentence["from"]]
        conv.append_message(role, sentence["value"])
    chosen = conv.get_prompt()

    # 构建 rejected（worst_conversations）
    conv.messages = []
    for j, sentence in enumerate(example['prompt'] + example['worst_conversations']):
        role = roles[sentence["from"]]
        conv.append_message(role, sentence["value"])
    ...
    # 计算最优路径和最差路径的嵌入向量
    if 'optimal_conversations' in example and 'worst_conversations' in example:
        optimal_text = " ".join([conv['value'] for conv in example['optimal_conversations']])
        worst_text = " ".join([conv['value'] for conv in example['worst_conversations']])
        optimal_embedding = sentence_transformer.encode(optimal_text).tolist()
        worst_embedding = sentence_transformer.encode(worst_text).tolist()
    else:
        # 如果没有路径信息，则使用全零向量作为默认值
        embedding_dim = sentence_transformer.get_sentence_embedding_dimension()
        optimal_embedding = [0.0] * embedding_dim
        worst_embedding = [0.0] * embedding_dim
    ...
    return {
        ...
        "optimal_path_embedding": optimal_embedding,
        "worst_path_embedding": worst_embedding,
    }
...
trainer = DPOMultiTrainer(
    similarity_weight=0.1,  # 控制相似度损失的权重
    model=policy_model,
    ref_model=ref_model,
    args=training_args,
    beta=beta,
